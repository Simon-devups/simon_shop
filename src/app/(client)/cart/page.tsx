...
/* ────────────────────────────────────────────────────────────────────────────
   Constants — STEP 2 will move شیپینگ/مالیات/کوپن به بک‌اند
   (POST /api/cart/coupon، POST /api/orders). فعلاً همینجا محاسبه می‌شن.
──────────────────────────────────────────────────────────────────────────── */
...
function LineItem({
  ...
  return (
    ...
    >
      {/* Thumbnail */}
      <div className="relative grid h-[104px] w-[88px] shrink-0 place-items-center overflow-hidden rounded-[16px] bg-[#F5F7FA] sm:h-[120px] sm:w-[104px]">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex items-start justify-between gap-3">
          ...
          <button
            ...
            onClick={onRemove}
            aria-label="حذف کالا"
            className="grid h-8 w-8 shrink-0 place-items-center rounded-[8px] text-[#9CA3AF] transition-colors duration-150 ease-out hover:bg-[#FDECEC] hover:text-[#EF4444]"
          >
            <Trash2 size={16} strokeWidth={2} />
          </button>
        </div>

        <p className="mt-1 text-xs text-[#6B7280]">{product.category.name}</p>
        ...
        <div className="mt-auto flex flex-wrap items-end justify-between gap-3 pt-3">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2">
              ...
              {discount > 0 && (
                <span className="rounded-[8px] bg-[#FDECEC] px-1.5 py-0.5 text-[11px] font-bold text-[#EF4444]">
                  {toFa(discount)}٪
                </span>
              )}
            </div>
...
/* ────────────────────────────────────────────────────────────────────────────
   Cart Page — الان از CartContext می‌خونه، نه یک آرایه‌ی هاردکد.
──────────────────────────────────────────────────────────────────────────── */
export default function CartPage() {
  ...
  const [checkout, setCheckout] = useState<"idle" | "loading" | "success">("idle");

  const discount = applied ? Math.round((subtotal * COUPON.percentOff) / 100) : 0;
  ...
  const applyCoupon = () => {
    // STEP 2: جایگزین با POST /api/cart/coupon
  ...
  const placeOrder = () => {
    // STEP 2: جایگزین با POST /api/orders
    if (items.length === 0 || checkout !== "idle") return;
    setCheckout("loading");
    ...
    <div dir="rtl" className="bg-[#F5F7FA]">
      <div className="mx-auto w-full max-w-[1600px] px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
        ...
        <div className="mb-6 lg:mb-10">
          <h1 className="text-xl font-extrabold text-[#111827] lg:text-2xl">سبد خرید شما</h1>
          <p className="mt-1 text-sm text-[#6B7280]">
            {empty ? "سبد خرید شما خالی است." : `${toFa(count)} کالا در سبد شما`}
          </p>
        </div>
        ...
        {!empty && subtotal < FREE_SHIPPING_THRESHOLD && (
          <div className="mb-6 flex flex-col gap-2 rounded-[16px] border border-[#E5E7EB] bg-white p-4 shadow-[0_8px_30px_rgba(0,0,0,.06)]">
            <div className="flex items-center gap-2 text-sm text-[#374151]">
              ...
              <span>
                با خرید <b className="text-[#1D4ED8]">{faPrice(remaining)}</b> دیگر ارسال رایگان شوید
              </span>
            </div>
        ...
        {empty ? (
          ...
          <div className="grid place-items-center rounded-[16px] border border-[#E5E7EB] bg-white px-6 py-16 text-center shadow-[0_8px_30px_rgba(0,0,0,.06)]">
            ...
            <h2 className="mt-6 text-lg font-bold text-[#111827]">سبد خرید شما خالی است</h2>
            <p className="mt-2 max-w-sm text-sm leading-relaxed text-[#6B7280]">
              هنوز محصولی به سبد خرید اضافه نکرده‌اید. برای مشاهده محصولات به فروشگاه بازگردید.
            </p>
            <button
              type="button"
              onClick={() => router.push("/")}
          ...
          <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-12">
              ...
              <ul className="overflow-hidden rounded-[16px] border border-[#E5E7EB] bg-white shadow-[0_8px_30px_rgba(0,0,0,.06)]">
                {items.map((item) => (
                  <LineItem
                    ...
                    onQuantity={(n) => updateQuantity(item.product.id, n)}
                  />
                ))}
              </ul>
            ...
            <aside className="lg:col-span-4">
              <div className="rounded-[16px] border border-[#E5E7EB] bg-white p-6 shadow-[0_8px_30px_rgba(0,0,0,.06)]">
                <h2 className="text-base font-bold text-[#111827]">خلاصه سفارش</h2>

                {/* Coupon */}
                <div className="mt-5 flex items-center gap-2">
                  <Ticket size={18} strokeWidth={2} className="shrink-0 text-[#1D4ED8]" />
                  <input
                    value={coupon}
                ...
                <dl className="mt-5 space-y-3 border-t border-[#F1F5F9] pt-5 text-sm">
                  <div className="flex items-center justify-between text-[#374151]">
                    <dt>جمع کالاها</dt>
                    <dd className="font-medium text-[#111827]">{faPrice(subtotal)}</dd>
                  </div>
                  {discount > 0 && (
                  ...
                  <div className="flex items-center justify-between text-[#374151]">
                    <dt>هزینه ارسال</dt>
                    <dd className={shipping === 0 ? "font-medium text-[#22C55E]" : "font-medium text-[#111827]"}>
                      {shipping === 0 ? "رایگان" : faPrice(shipping)}
                    </dd>
                  </div>
                  <div className="flex items-center justify-between text-[#374151]">
                    <dt>مالیات (۹٪)</dt>
                    <dd className="font-medium text-[#111827]">{faPrice(tax)}</dd>
                  </div>

                  <div className="flex items-center justify-between border-t border-[#F1F5F9] pt-4">
                    <dt className="text-base font-bold text-[#111827]">مبلغ قابل پرداخت</dt>
                    <dd className="text-lg font-extrabold tracking-tight text-[#1D4ED8]">{faPrice(total)}</dd>
                  </div>
                </dl>
                ...
                >
                  ...
                  {checkout === "success" && <Check size={18} strokeWidth={2.5} />}
